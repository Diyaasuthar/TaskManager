...
const SignUp = ({onSwitchMode}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setMessage({text: "Registration successfull! You can nov log in.", type: "success"})
        ...
        <div className="max-w-md w-full bg-white shadow-lg border border-purple-100 rounded-xl p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button type="submit" className={BUTTONCLASSES} disabled={loading}>
                    {loading ? "Signin Up..." : <><UserPlus className="w-4 h-4" />Sign Up</>}
                </button>
            </form>
            <p className="text-center text-sm text-gray-600 mt-6">
                Already have an account?{' '}
                <button onClick={onSwitchMode} className="text-purple-600 hover:text-purple-700 hover:underline font-medium trasition-colors">
                    Login
                </button>
            </p>
        </div>
