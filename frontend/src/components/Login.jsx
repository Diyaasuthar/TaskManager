...
const fields = [
    ...
    return (
        <div className="max-w-md bg-white w-full shadow-lg border border-purple-100 rounded-xl p-8">
            <ToastContainer position="top-center" autoClose={3000} highProgressBar />
                ...
                {fields.map(({name, type, placeholder, icon:Icon, isPassword })=>(
                    <div key={name} className={INPUTWRAPPER}>
                        <Icon className="text-purple-500 w-5 h-5 mr-2" />
                         ...
                         {isPassword && (
                            <button type="button" onClick={()=>setShowPassowrd((prev) => !prev)}
            ...
            <p className="text-center text-sm text-gray-600 mt-6">
                ...
                <button type="button" className="text-purple-600 hover:text-shadow-purple-700 hover:underline font-medium transition-colors" onClick={handleSwitchMode}>Sign Up</button>
            </p>
        </div>
    )
}
